...
Each Stage 2 token is built directly from Stage 1 character IDs.
...
is not present in Stage 2, but all of its characters are known, it can fall back to:
...
"<UNK>" is only required when the tokenizer encounters a character that was not present in the learned Stage 1 character vocabulary.

This provides a complete fallback representation for any word composed entirely of known characters.
...
Vocabulary generation is deterministic.
...
The tokenizer supports memory-efficient corpus processing through streaming.

Large corpora can be processed without requiring the entire corpus to be loaded into memory at once.
...
The important architectural property is that Stage 2 does not replace Stage 1.

Instead, Stage 2 provides a compact representation for selected tokens while Stage 1 remains the underlying fallback representation.
...
if present in Stage 2:

play → [PLAY_ID]

If "play" is not present:

play → [WORD_BOUND, p, l, a, y]
...
Characters
   ...
   ▼
Stage 2
Frequent multi-character tokens

This removes the iterative merge-loop architecture from vocabulary construction.

The result is a simpler and more directly auditable tokenization process.
...
Every Stage 2 token maintains its Stage 1 recipe.
...
The intended purpose is to provide a cache of frequently occurring combinations of Stage 2 tokens.
